...
fn test_bigint_literals_and_arithmetic() {
    assert_eq!(
        compile_and_run("42n").unwrap(),
        Value::BigInt(BigInt::from(42))
    );
    assert_eq!(
        compile_and_run("-7n").unwrap(),
        Value::BigInt(BigInt::from(-7))
    );
    assert_eq!(
        compile_and_run("0n").unwrap(),
        Value::BigInt(BigInt::from(0))
    );

    // Spec: 10n / 3n == 3n, 10n % 3n == 1n
    assert_eq!(
        compile_and_run("10n + 20n").unwrap(),
        Value::BigInt(BigInt::from(30))
    );
    assert_eq!(
        compile_and_run("50n - 30n").unwrap(),
        Value::BigInt(BigInt::from(20))
    );
    assert_eq!(
        compile_and_run("6n * 7n").unwrap(),
        Value::BigInt(BigInt::from(42))
    );
    assert_eq!(
        compile_and_run("100n / 7n").unwrap(),
...
fn test_float_literals_and_arithmetic() {
    ...
    assert_eq!(
        compile_and_run("2.5f32").unwrap(),
        Value::Float(2.5_f32 as f64)
    ); // f32 stored as f64
    ...
    assert_eq!(
        compile_and_run("1.5f64 + 2.25f64").unwrap(),
        Value::Float(3.75)
    );
    assert_eq!(
        compile_and_run("5.0f64 - 3.0f64").unwrap(),
        Value::Float(2.0)
    );
    assert_eq!(
        compile_and_run("2.0f64 * 3.0f64").unwrap(),
        Value::Float(6.0)
    );
    assert_eq!(
        compile_and_run("10.0f64 / 4.0f64").unwrap(),
        Value::Float(2.5)
    );
}
...
fn test_extended_type_comparisons() {
    ...
    assert_eq!(
        compile_and_run("3.14f64 == 3.14f64").unwrap(),
        Value::Bool(true)
    );
    assert_eq!(
        compile_and_run("1.0f64 < 2.0f64").unwrap(),
        Value::Bool(true)
    );
