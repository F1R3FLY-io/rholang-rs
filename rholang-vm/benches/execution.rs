...
fn execution(bencher: divan::Bencher, arg: &PathBuf) {
    ...
    // Compile the code outside the benchmark
    let bytecode = vm.compile(&code).expect("failed to compile code");
    
    // Create a runtime for executing async code
    let rt = Runtime::new().expect("failed to create runtime");
    
    bencher.bench_local(|| {
        // Execute the bytecode in the runtime
